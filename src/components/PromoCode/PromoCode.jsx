...
const PromoCodeTable = () => {
    ...
    useEffect(() => {
        if (tableRef.current && currentItems.length > highlightedRowIndex) {
            ...
            if (targetRow) {
                ...
                const topPosition = rowRect.top - tableRect.top + tableRef.current.scrollTop + rowRect.height / 2 - 12; // 12 is half of 24px icon height
    ...
    const handleExtend = () => {
        if (!isExtendDisabled && selectedDate) {
            const updatedPromoCodes = promoCodes.filter((_, idx) => idx !== indexOfFirst + highlightedRowIndex);
            // Here you would typically update the state or backend, but for this example, we'll just log
            console.log("Extended and removed row:", indexOfFirst + highlightedRowIndex);
    ...
    return (
        <div className="relative">
            ...
            <div className="flex justify-between items-start">
                ...
                <div>
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => navigate('/dashboard/promocode/createpromo')} className="flex items-center gap-2 px-4 py-2 bg-white rounded-md text-sm font-medium border shadow-md
            ...
            {/* Table wrapper */}
            <div className="relative p-2" style={{ overflow: 'visible' }}> {/* Ensure no clipping */}
                {/* Outside side icon */}
                    ...
                    <tbody>
                        {currentItems.map((item, idx) => {
                            ...
                            return (
                                <tr
                                    key={item.id}
                                    className={`border-b text-sm ${isHighlighted ? "bg-red-100" : idx % 2 === 0 ? "bg-white" : "bg-gray-50"
                                        }`}
                                >
                                    ...
                                    <td className="px-4 py-2 flex justify-center gap-3">
                                        <button onClick={() => navigate('/dashboard/promocode/editcode')} className="text-[#8A1538] hover:text-black">
                                            <FaEdit />
                                        </button>
            ...
            {showFirstModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white p-6 rounded-lg shadow-lg">
                        ...
                        <p>Do you want to extend and keep using this promo code <br /> going forward?</p>
                        <div className="mt-4 flex justify-center gap-4">
                            <button className="px-4 py-2 bg-[#8A1538] text-white rounded" onClick={handleNoClick}>No</button>
                            <button className="px-4 py-2 bg-[#0A8910] text-white rounded" onClick={handleYesClick}>Yes</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {showSecondModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white p-6 rounded-lg shadow-lg">
                        <p className="text-[#8A1538] font-bold">Promo Code Expired...!!!</p>
                        <p>Delete this code permanently? Once deleted, it cannot be <br /> recovered.</p>
                        <div className="mt-4 flex justify-center gap-4">
                            <button className="px-4 py-2 bg-[#BC0101] text-white rounded" onClick={handleDeleteForever}>Delete Forever</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="flex justify-between items-center mt-4 text-sm">
                <p>
                    Showing {indexOfFirst + 1} to {Math.min(indexOfLast, promoCodes.length)} of{" "}
                    {promoCodes.length} entries
                </p>
